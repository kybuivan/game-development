...
void BulletHandler::updateBullets()
{
    for (std::vector<PlayerBullet*>::iterator p_it = m_playerBullets.begin(); p_it != m_playerBullets.end();)
    {
        if((*p_it)->getPosition().getX() < 0 || (*p_it)->getPosition().getX() > Game::Instance()->getGameWidth()
           || (*p_it)->getPosition().getY() < 0 || (*p_it)->getPosition().getY() > Game::Instance()->getGameHeight() || (*p_it)->dead())
        {
            delete * p_it;
    ...
    {
        if((*e_it)->getPosition().getX() < 0 || (*e_it)->getPosition().getX() > Game::Instance()->getGameWidth()
           || (*e_it)->getPosition().getY() < 0 || (*e_it)->getPosition().getY() > Game::Instance()->getGameHeight() || (*e_it)->dead())
        {
            delete * e_it;
